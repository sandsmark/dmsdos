...
int sq_dec(void *pin,int lin, void *pout, int lout, int flg)
{
    ...
    do {
        final_flag=sq_rdn(&bits,1);
        LOG_DECOMP("DMSDOS: final_flag %d\n",final_flag);
        method=sq_rdn(&bits,2);
        LOG_DECOMP("DMSDOS: method %d\n",method);
        ...
        case 2:
            ...
            while (i<19) { code_bln[code_index_1[i++]]=0; }
...
__u16 sq_comp_rat_tab[]= {
    ...
    0x46E9
};                     /* compression ratio to seek lengths */
